import {
    parse, QM, ParamType, ParamCritType,
    Location, Jump, ParameterShowingType
} from '../lib/qmreader';
        ...
        describe('Param definitions', () => {
            ...
            describe('Locations', () => {
                ...
                it('Visit limit loc id=10', () => {
                    assert.strictEqual(qm.locations.find(x => x.id === 10)!.maxVisits,
                        ext === 'qmm' ? 78 : 0)
                })
                it('Visit limit loc id=5', () => {
                    assert.strictEqual(qm.locations.find(x => x.id === 5)!.maxVisits,
                        ext === 'qmm' ? 312 : 0)
                })
