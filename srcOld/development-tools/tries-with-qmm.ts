...
const qmmDir = 'c:\\R.G. Catalyst\\Space Rangers HD A War Apart2.1.2170\\DATA\\questsRus\\Data\\Quest\\Rus\\'
//const qmDir = __dirname + '/../../data/src/rus/'
const qmDir = 'c:\\R.G. Catalyst\\Space Rangers HD A War Apart\\DATA\\questsRus\\Data\\Quest\\Rus\\'
for (const fileName of fs.readdirSync(qmmDir)) {
    ...
    if (diff) {
        const diffWithoutAddings = diff.filter((x: any) => !(
            (x.kind === 'A' && x.item.kind === 'N' &&
