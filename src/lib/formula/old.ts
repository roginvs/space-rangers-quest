...
const operations: Operation[] = [
  ...
  },
  {
    ...
    fBinary: (a, b, random) => {
      ...
      const rangeAmax = rangeA.reduce(
        (max, range) => (range[1] > max ? range[1] : max),
        0
      );
      const rangeBmax = rangeB.reduce(
        (max, range) => (range[1] > max ? range[1] : max),
        0
      );
      const rangeAmin = rangeA.reduce(
        (min, range) => (range[0] < min ? range[0] : min),
        MAX_NUMBER
      );
      const rangeBmin = rangeB.reduce(
        (min, range) => (range[0] < min ? range[0] : min),
        MAX_NUMBER
      );
...

  {
    ...
    prio: 7,
    fBinary: (a, b, random) =>
      argToNumber(a, random) >= argToNumber(b, random) ? 1 : 0
  },
  {
    longSymbol: "<=",
    shortSymbol: "b",
...
function findClosingBrackedIndex(
  str: string,
  openBrackedIndex: number,
  type: "round" | "square"
) {
  ...
  if (str[openBrackedIndex] === open) {
    ...
    throw new Error(
      `Closed bracked not found: str='${str}', openIndex=${openBrackedIndex}, opencount=${opencount}`
    );
  } else {
...
function parseRecursive(
  ...
  if (str[0] === "[" && str[1] === "p" && str.indexOf("]") === str.length - 1) {
    ...
    if (!oper) {
      debug(
        deep,
        `String '${str}', operands are: ` +
          operands
            .map(
              x =>
                `pos=${x.pos} symbol=${x.operation.longSymbol}/${x.operation.shortSymbol}`
            )
            .join("; ")
      );
    ...
    debug(
      deep,
      `String '${str}' have operand ${oper.operation.longSymbol}, ` +
        `will parse '${a}' and '${b}'`
    );
    if (a) {
      ...
      debug(
        deep,
        `String '${str}' have one operand ${oper.operation.longSymbol}, ` +
      ...
      if (oper.operation.fBinary) {
        ...
        throw new Error(
          `Usage of ${oper.operation.longSymbol} as Unary in ${str}`
        );
      }
    } else {
