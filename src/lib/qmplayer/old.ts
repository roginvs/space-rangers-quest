...
class QMPlayer {
  ...
  public getState(): PlayerState {
    if (this.state.state === "starting") {
      ...
      const jump = this.quest.jumps.find(x => x.id === this.state.lastJumpId);
      if (!jump) {
        throw new Error(
          `Internal error: no last jump id=${this.state.lastJumpId}`
        );
      }
      return {
      ...
      if (!location) {
        throw new Error(
          `Internal error: no state loc id=${this.state.locationId}`
        );
      }
          ...
          this.state.state === "location"
            ? location.isFaily || location.isFailyDeadly
              ...
              : this.state.possibleJumps.map(x => {
                  const jump = this.quest.jumps.find(y => y.id === x.id);
                  if (!jump) {
                    throw new Error(
                      `Internal error: no jump ${x.id} in possible jumps`
                    );
                  }
                  return {
                    text: this.replace(jump.text) || this.texts.next,
                    jumpId: x.id,
                    active: x.active
                  };
                })
      ...
      if (critId === undefined || !jump) {
        throw new Error(
          `Internal error: crit=${critId} lastjump=${this.state.lastJumpId}`
        );
      }
      ...
      if (!location) {
        throw new Error(
          `Internal error: no crit state location ${this.state.locationId}`
        );
      }
      const param = this.quest.params[critId];
          ...
          const image = qmmImage
            ...
            : this.images
                .filter(
                  x =>
                    !!x.critParams &&
                    x.critParams.indexOf(this.state.critParamId as number) > -1
                )
                .map(x => x.filename)
                .shift();
          if (image) {
  ...
  private calculateLocationShowingTextId(location: Location) {
    ...
    if (location.textSelectFurmula) {
      ...
      return (
        (locationTextsWithText[textNum] && locationTextsWithText[textNum].i) ||
        0
      );
    }
  }
  private calculateLocation() {
    ...
    if (!location) {
      throw new Error(
        `Internal error: no state location ${this.state.locationId}`
      );
    }
    ...
    const imageFromPQI = this.images.find(
      x => !!x.locationIds && x.locationIds.indexOf(this.state.locationId) > -1
    );
        ...
        if (this.oldTgeBehaviour) {
          ...
          if (
            jumpsFromDestination.filter(
              x =>
                x.jumpingCountLimit &&
                this.state.jumpedCount[x.id] >= x.jumpingCountLimit
          ...
          active: (jump => {
            for (let i = 0; i < this.quest.paramsCount; i++) {
              if (this.quest.params[i].active) {
                if (
                  this.state.paramValues[i] > jump.paramsConditions[i].mustTo ||
                  this.state.paramValues[i] < jump.paramsConditions[i].mustFrom
                ) {
                  return false;
                }
            ...
            if (
              ...
              this.state.jumpedCount[jump.id] >= jump.jumpingCountLimit
            ) {
              return false;
            }
            return true;
    ...
    for (const critParam of critParamsTriggered) {
      const gotCritWithChoices =
        (this.quest.params[critParam].type === ParamType.Провальный ||
          this.quest.params[critParam].type === ParamType.Смертельный) &&
      ...
      if (!this.oldTgeBehaviour || !gotCritWithChoices) {
        ...
        const image = qmmImg
          ...
          : this.images
              .filter(
                x => !!x.critParams && x.critParams.indexOf(critParam) > -1
              )
              .map(x => x.filename)
              .shift();
        if (image) {
    ...
    if (state.choices.length === 1) {
      const jump = this.quest.jumps.find(x => x.id === state.choices[0].jumpId);
      const lastjump = this.quest.jumps.find(
        x => x.id === this.state.lastJumpId
      );
      const location = this.quest.locations.find(
      ...
      if (
        ...
        console.info(
          `Performinig autojump from loc=${this.state.locationId} via jump=${jump.id}`
        );
