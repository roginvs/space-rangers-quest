...
type HeaderMagic = 0x423A35D6 | 0x423A35D7 |
    0x423A35D3 | 0x423A35D2 | 0x423A35D4;
...
function parseBase(r: Reader, header: HeaderMagic): QMBase {
    ...
    if (header === 0x423A35D6 || header === 0x423A35D7) {
        ...
        const changeLogString = header === 0x423A35D7 ? r.readString() : undefined;
        ...
        return {
            givingRace, whenDone, planetRace, playerCareer,
            playerRace, defaultJumpCountLimit, hardness,
            paramsCount,
            changeLogString, majorVersion, minorVersion
        }
    } else {
        const paramsCount = header === 0x423A35D3 ? 48 :
            header === 0x423A35D2 ? 24 :
        ...
        const hardness = r.int32();
        return {
            givingRace, whenDone, planetRace, playerCareer, playerRace, defaultJumpCountLimit, hardness,
            paramsCount,
        }
    }
}
...
function parseParam(r: Reader): QMParam {
    ...
    const param: QMParam = {
        min, max, type, showWhenZero, critType, active, showingRangesCount, isMoney, name,
        showingInfo: [],
        starting: '',
        critValueString: '',
        img: undefined, sound: undefined, track: undefined
    }
...
function parseBase2(r: Reader, isQmm: boolean): QMBase2 {
    ...
    return {
        strings: { ToStar, Parsec, Artefact, ToPlanet, Date, Money, FromPlanet, FromStar, Ranger },
        locationsCount,
        jumpsCount,
        successText,
        taskText
...
function parseLocation(r: Reader, paramsCount: number): Location {
    ...
    for (let i = 0; i < paramsCount; i++) {
        ...
        paramsChanges.push({
            change, showingType, isChangePercentage, isChangeValue, isChangeFormula, changingFormula, critText,
    ...
    return {
        dayPassed, id, isEmpty, isFaily, isFailyDeadly, isStarting, isSuccess,
        paramsChanges,
        texts, media,
        isTextByFormula, textSelectFurmula,
        maxVisits: 0
    }
}
function parseLocationQmm(r: Reader, paramsCount: number): Location {
    ...
    return {
        ...
        paramsChanges,
        texts, media,
        isTextByFormula, textSelectFurmula, maxVisits
    }
}
...
function parseJumpQmm(r: Reader, paramsCount: number, questParams: QMParam[]): Jump {
    ...
    for (let i = 0; i < affectedConditionsParamsCount; i++) {
        ...
        paramsConditions[paramId - 1] = {
            mustFrom, mustTo,
            mustEqualValues, mustEqualValuesEqual,
            mustModValues, mustModValuesMod
        }
    }
    ...
    for (let i = 0; i < affectedChangeParamsCount; i++) {
        ...
        paramsChanges[paramId - 1] = {
            change, showingType,
            isChangeFormula, isChangePercentage, isChangeValue,
            changingFormula,
            critText,
            img, track, sound
        }
...
export function parse(data: Buffer): QM {
    ...
    for (let i = 0; i < base2.locationsCount; i++) {
        locations.push(isQmm ? parseLocationQmm(r, base.paramsCount) :
