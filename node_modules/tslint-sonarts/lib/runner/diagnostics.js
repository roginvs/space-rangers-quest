...
function getDiagnostics(sourceFile, program) {
    ...
    return diagnostics.filter(diagnostic => diagnostic.category === ts.DiagnosticCategory.Error).map(diagnostic => {
        ...
        const { line, character } = navigation_1.lineAndCharacterByPos(diagnostic.start, sourceFile);
        return {
            message,
            line: sonarUtils_1.toSonarLine(line),
            col: character,
        };
    });
}
