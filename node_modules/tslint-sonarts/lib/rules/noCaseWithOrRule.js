...
class Visitor extends sonarAnalysis_1.SonarRuleVisitor {
    visitCaseClause(node) {
        ...
        if (chain !== undefined) {
            ...
            const left = nodes_1.isStringLiteral(chain.left) ? chain.left.getText() : `"${chain.left.getText()}"`;
            this.addIssue(node.expression, `Explicitly specify ${chain.count} separate cases that fall through; ` +
                `currently this case clause only works for ${left}.`);
        }
        super.visitCaseClause(node);
    }
    pickBarBarChain(expression) {
        let current = expression;
        let count = 0;
        while (nodes_1.isBinaryExpression(current) && current.operatorToken.kind === ts.SyntaxKind.BarBarToken) {
            count++;
...
//# sourceMappingURL=noCaseWithOrRule.js.map